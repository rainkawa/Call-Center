...
// ==================== SOUND SYSTEM ====================
// Some Android WebViews expose no AudioContext (or only a prefixed one), and
// constructing it unconditionally would abort the whole script — killing the
// start menu buttons. Guard it so the game never dies on audio.
let audioCtx = null;
try {
...
function initStartMenu() {
    ...
    // localStorage can throw (private mode, restricted WebView). Never let it
    // block the start buttons — wrap every access defensively.
    let hasSave = false;
    let saveDay = 1;
    try {
    ...
    if (hasSave && continueBtn) {
        ...
        continueBtn.textContent = '▶️ Continue (Day ' + saveDay + ')';
    }
    ...
    newGameBtn.addEventListener('click', () => {
        ...
        } catch (e) { /* ignore — save is cleared in-memory below */ }
        G.cash = 500; G.leads = 50; G.reputation = 75; G.day = 1;
        G.upgrades = {}; G.hasSupervisor = false; G.hasCoffee = false;
        G.totalRevenue = 0; G.totalSales = 0; G.agents = [];
        isNewGame = true;
        startGame();
    });
}
...
function startGame() {
    ...
    // Guard the whole startup so a hiccup (WebGL unavailable, an init error)
    // can never leave the loading screen frozen on the start menu.
    try {
        ...
        const status = document.getElementById('loading-screen')
            ?.querySelector('.loading-subtitle') ||
            document.getElementById('loading-screen');
        if (status) {
            status.textContent = 'Could not start 3D — try reloading (WebGL may be off).';
        }
        setTimeout(() => {
...
window.getCurrentPad = () => currentPad;
...
function createLights() {
    ...
    dir.castShadow = true;
    dir.shadow.mapSize.set(IS_MOBILE_DEVICE ? 1024 : 2048, IS_MOBILE_DEVICE ? 1024 : 2048);
...
// Mobile controls access the REAL game input state.
// No synthetic KeyboardEvents required.
window.gameKeys = keys;
window.keys = keys;

/* Mobile controls access */
...
function updateCamera() {
    ...
    camera.position.x += (targetX - camera.position.x) * (IS_MOBILE_DEVICE ? 0.12 : 0.08);
    camera.position.z += (targetZ - camera.position.z) * (IS_MOBILE_DEVICE ? 0.12 : 0.08);
    camera.position.y += (targetY - camera.position.y) * (IS_MOBILE_DEVICE ? 0.12 : 0.08);
